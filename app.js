...
function fnMatchSearch(targetCard, searchCondition) {
    ...
    if (searchCondition.level) {
        if (searchCondition.level.max && targetCard.level > searchCondition.level.max) return false;
        if (searchCondition.level.min && targetCard.level < searchCondition.level.min) return false;
    }

    if (searchCondition.type) {
        const isTypeMatches = searchCondition.type.every(t => targetCard.type && targetCard.type.includes(t));
        if (!isTypeMatches) return false;
    }
    
    if (searchCondition.att) if (searchCondition.att != targetCard.att) return false;
    if (searchCondition.card) if (searchCondition.card != targetCard.card) return false;
    if (searchCondition.name) if (searchCondition.name != targetCard.name) return false;
...
window.onload = async function() {
    try {
        ...
        inputField.addEventListener("input", function() {
            ...
            const matches = cardDatabase.filter(c => c.name.toLowerCase().includes(val.toLowerCase()));
...
function selectAndDisplayCard(cardName) {
    ...
    let scaleStr = selectedCard.scale !== undefined ? `스케일 ${selectedCard.scale}` : "";
    
    profileHtml += `<p class="card-line-1">${levelRankLinkStr} ${attrStr} ${scaleStr}</p>`;

    // 두 번째 줄: [목록] 공격력 / 수비력
    
    let typeListStr = Array.isArray(selectedCard.type) ? selectedCard.type.join(' / ') : (selectedCard.type || '-');
    
    let atkDefStr = selectedCard.atk !== undefined ? `${selectedCard.atk} / ${selectedCard.def}` : "";
    ...
    // 네 번째 줄: 텍스트 (줄바꿈 유지)
    let cardText = selectedCard.text || selectedCard.desc || "상세 텍스트가 존재하지 않습니다.";
    ...
    cardDatabase.forEach(other => {
        if (selectedCard.search) {
            if (fnMatchSearch(other, selectedCard.search) && !fnMatchSearch(other, selectedCard.exception)) {
    ...
    cardDatabase.forEach(other => {
        if (other.search) {
            if (fnMatchSearch(selectedCard, other.search) && !fnMatchSearch(other, selectedCard.exception)) {
                let li = document.createElement("li");
                li.textContent = other.name;
                li.onclick = () => {
                    selectAndDisplayCard(other.name);
